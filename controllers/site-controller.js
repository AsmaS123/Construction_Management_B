const {Site,ClientExpenses,Contractor,ContractorExpenses} = require("../models/model");
const crypto = require("crypto");
const {  ObjectId } = require('mongodb');

const createSite = async(req,res,next) =>{
    const { site_type, site_name, site_engineer_name, site_engineer_email,
            client_name,client_email, client_contact_no, client_expenses,client_expenses_status,client_expenses_date,
            site_address,  site_start_date, site_end_date,
            contractor} = req.body;
    let existingSite;
    let result;
    try{
        ...
        if(existingSite){
         ...
         const site = new Site({
            site_id , site_type, site_name, site_engineer_name, site_engineer_email,
            client_name,client_email, client_contact_no, client_expenses, client_expenses_status,
            site_address, site_start_date, site_end_date,
            contractor
        });
...
const updateSite = async(req,res,next) =>{
    ...
    try{
        const result =  await Site.updateOne({_id: objectId},{$set: {...rest}})
...
const updateSiteExpenses = async(req,res,next) =>{
    const {_id, site_id, client_expenses,client_expenses_status,client_expenses_date,
        contractor} = req.body;
        // console.log(req.body,'req.body')
        const objectId = new ObjectId(_id); // Convert to ObjectId
    
        try{
            const result =  await Site.updateOne({_id: objectId},{$set: {    
...
const siteList = async(req,res,next) =>{
    ...
    let siteList;
    try{
        siteList = await Site.find({},{site_id:1, site_type:1, site_name:1, site_engineer_name:1, site_engineer_email:1, site_address:1,site_start_date:1,_id:0});
...
const expensesList = async(req,res,next) =>{
    ...
    try{
        ...
        expensesDetail = await Site.findOne({site_id:siteid},{site_id:1,site_type:1,site_name:1,site_engineer_name:1, client_name:1, client_expenses:1, client_expenses_status:1, client_expenses_date:1,contractor:1});
...
module.exports = { createSite,updateSite, siteList, siteDetails, expensesList ,updateSiteExpenses};